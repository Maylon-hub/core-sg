...
class TestGraphHelpers:
    ...
    def test_kruskal_mst_preserves_signature(self):
        assert str(inspect.signature(kruskal_mst)) == "(edges: 'np.ndarray', n_nodes: 'int') -> 'np.recarray'"
    ...
    def test_kruskal_mst_preserves_disconnected_graph_error(self):
        ...
        with pytest.raises(ValueError, match="Disconex Graph: MST extraction is not possible."):
    ...
    def test_reweight_matches_python_backend(self, core_sg_module):
        ...
        metric_edges = np.array([[1, 0, 1.0], [2, 1, 4.0], [2, 0, 2.5]], dtype=np.float64)
        core_k = np.array([0.5, 2.0, 3.0], dtype=np.float64)
        ...
        assert np.array_equal(weighted_accelerated, core_sg_module.sort_core_sg(weighted_python))
