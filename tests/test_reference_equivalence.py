...
def test_reference_mst_edges_are_contained_in_core_sg(dataset, k):
    core = CoreSG(metric="euclidean", p=2, debug=False, match_reference_implementation=True)
    ...
    assert summary.ok, f"Missing {summary.missing} reference edges out of {summary.compared}."
...
def test_reference_mrd_weights_are_present_in_reweighted_core_sg(dataset, k):
    ...
    assert summary.ok, f"Missing {summary.missing} weighted reference edges out of {summary.compared}."
...
def test_reference_and_core_sg_mst_are_reported_without_failing(dataset, k, capsys):
    ...
    same_weighted_edges = normalize_undirected_edges(mst_hdb) == normalize_undirected_edges(mst_core)
