...
def hdbscan_reference_mst_original_distance(D: np.ndarray, **kwargs) -> np.ndarray:
    ...
    clusterer = hdbscan.HDBSCAN(
        min_cluster_size=2,          # qualquer >=2 (não importa, você só quer a MST)
        min_samples=1,
        metric="precomputed",
        algorithm="generic",
...
def build_core_sg_from_data(
    ...
    **hdbscan_kwargs,
    ...
    # core-distance correta do HDBSCAN:
    ...
    mst_orig = None
    core_sg = knng_to_insert

    if include_mst_edges:
        ...
        print("mst_orig max diff:", np.max(np.abs(mst_orig[:, 2] - D[u, v])))
...
def mst_and_single_linkage_from_core_sg(
    core_sg: np.ndarray,
